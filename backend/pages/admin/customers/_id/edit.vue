<template>
	<loader :loading="$fetchState.pending">
		<form-view @submitted="save" title="Edit Customer" :breadcrumb="breadcrumb">
			<template #form>
				...
				<form-group>
					...
					<div class="w-full mt-1">
						<form-image-input :url="form.profilePhotoUrl" v-model="form.image"></form-image-input>
			...
			<template #actions>
				<nuxt-link to="/admin/customers" class="xs:mr-4 text-sm">Cancel</nuxt-link>
...
export default {
	...
	data() {
		return {
			...
			breadcrumb: [
				...
				{ label: "Products", route: "/admin/customers" },
				{
					label: this.$route.params.id,
					route: `/admin/customers/${this.$route.params.id}`,
				},
	...
	async fetch() {
		...
		this.setValues(this.form, response.data);
	},
};
</script>
